...
set -e
cd "$(dirname "$0")"
FF=bin/ffmpeg-master-latest-linux64-gpl/bin/ffmpeg   # modern ffmpeg (xfade, lanczos, unsharp)
W=render/_feat
mkdir -p "$W" render/mp4
DUR=9.4

# 1) animated transparent overlay (window chrome + living bg + popping tags/icons; hole transparent)
node render_overlay_seq.mjs scenes/features_scene.html "$DUR" feat_overlay 30

# 2) three clip segments -> window size, quality up (lanczos + denoise + unsharp)
...
# 3) composite: black base + window video at hole + animated transparent overlay on top
"$FF" -hide_banner -loglevel error \
  -f lavfi -i "color=c=black:s=1920x1080:r=30:d=$DUR" \
  -i "$W/window.mp4" -framerate 30 -i "render/frames/feat_overlay/f%04d.png" \
